...
display(len(cdc_no_missing_df.columns))
...
MentHlth_cutoff = -40
cdc_no_outliers_df = cdc_no_missing_df.filter(col("MentHlth") <= MentHlth_cutoff)
...
print(f"count before removing outliers: {cdc_no_missing_df.count()} --> {cdc_no_outliers_df.count()}") 
...
test_df.write.mode("overwrite").option("overwriteSchema", True).saveAsTable(f"{DA.catalog_name}.{DA.schema_name}.cdc_df_test")
...
string_cols = [column.name for column in train_df.schema.fields if (column.dataType == StringType())]
num_cols = [column.name for column in train_df.schema.fields if (column.dataType == DoubleType())]
